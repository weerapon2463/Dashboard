...
function paperFooter(formCode) {
  const now = new Date();
  const fs = formSettings();
  return `<div class="paper-foot"><span>${escapeHtml(formCode)}</span>${fs.footer ? `<span>${escapeHtml(fs.footer)}</span>` : ""}<span>ออกจาก ${APP_NAME} · ${formatThaiDate(now.toISOString().slice(0, 10))}</span></div>`;
}
